...
Request preprocessing removes complete known Desktop browser-context and question-response envelopes before sentence extraction. Reply-only inputs (including recognized bare response arrays) stop backward input selection and use the current answer topic. Unclosed known envelopes are discarded to the bounded input end. Other Markdown and JSON retain the existing filtering behavior; no session-history lookup or additional model call is introduced.
