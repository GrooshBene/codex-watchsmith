...
공식 정식 릴리스를 확인하고 패키지를 검증한 뒤, 기존 설치기와 복구 기록을 사용합니다. 최초 이전 이후에는 Git이나 저장소 폴더가 없어도 업데이트할 수 있습니다. v0.1.0에는 이 명령이 없으므로 기능이 포함된 릴리스까지 한 번은 기존 방식으로 업그레이드해야 합니다. 이번 구현만으로 새 릴리스가 공개되는 것은 아닙니다.
