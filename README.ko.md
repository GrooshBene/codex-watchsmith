...
### 휴대폰에서 다음 작업을 선택하세요 — 개발 브랜치 미리보기

[notification-scenarios 브랜치](https://github.com/GrooshBene/codex-watchsmith/tree/codex/feat-notification-scenarios)에는 승인 버튼을 바로 보여주는 후속 작업 흐름이 추가돼 있습니다. 진단 요청에서 **검증 진행**을 선택하면 명시된 읽기 전용 명령을 실행하고, **여기서 종료**를 선택하면 실행하지 않습니다. 이미 허용한 작업에 다시 승인을 요구하지 않습니다.

![watchsmith doctor --json 읽기 전용 진단의 검증 진행·여기서 종료 선택지를 보여주는 개발 기능 예시.](docs/assets/usecases-approval-ko.svg)
...
- **설치하는 버전을 확인하세요.** 이 README는 main 브랜치의 기본 동작을 설명합니다. 아직 병합되지 않은 기능 브랜치는 안정 릴리즈 다운로드에 포함되지 않습니다.
