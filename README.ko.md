...
분석·계획·구현·검증·빌드·전송·설치·배포·복구·사용자 선택·최종 결과를 29개 상황으로 구분합니다. 실제 측정값이 있으면 진행률·단계·수치를, 없으면 경과 시간을 표시합니다. “커밋 진행 / 여기서 종료”, “재시도 / 중단” 같은 선택은 ActivitySmith의 기존 승인 요청을 활용합니다.

메시지 helper는 로컬에서 인수를 만들고, 관리 지침은 활성 에이전트가 같은 질문의 응답을 기다린 뒤 허용된 후속 작업을 처리하도록 안내합니다. 이미 허용된 작업에 확인을 추가하지 않습니다. 완료 Push는 기존 hook을 유지합니다. 새 서버·앱 종료 후 자동 재개·Codex 자체 권한 승인 우회는 제공하지 않습니다. [전체 상황과 예시](docs/MESSAGE_SCENARIOS.md)를 참고하세요. v0.2.3에는 포함되지 않으며 개발 버전 설치 후 적용됩니다.

선택 알림은 wrapper 안에서도 버튼이 바로 보이는 Live Activity를 기본으로 사용합니다. 선택 대기 동안 진행 표시를 잠시 멈추고, 후속 작업을 계속하면 같은 키·타입으로 복귀합니다. Push·자동 대체를 명시적으로 사용하는 경우에는 길게 눌러 선택하라는 안내를 포함합니다. 전달이나 응답이 불명확하면 승인으로 처리하지 않습니다.
완료 알림 제목에서 Desktop의 브라우저 자동 정보와 확인 질문 응답 블록을 제외합니다. 확인 응답만 있는 경우에는 내부 JSON이나 이전 요청 대신 현재 답변의 작업 결과를 사용합니다. 이는 제한된 문장 추출이며 의미 기반 요약은 아닙니다.
